...
try:
    ...
    from transferattack.advanced_objective.p2fa import P2FA as TransferAttackP2FA
except ImportError as e:
    logging.error(
        "FATAL: Failed to import 'MIFGSM' or 'P2FA'."
    )
    logging.error(
...
class P2FA(BaseAttack):
    ...
    def __init__(self, model, eps, alpha, steps, decay=1.0,  num_ens=30, feature_layer= 'layer2', eta=28.0):
        """
        ...
        """
        logging.info("Initializing TransferAttack P2FA...")
        super().__init__(model)
        autual_feature_layer = f"model.{feature_layer}"
        ...
        self.feature_layer = autual_feature_layer
        self.eta = eta
        ...
        logging.info(
            f"Initialized TransferAttack P2FA with parameters: "
            f"epsilon={self.epsilon:.4f}, alpha={self.alpha:.4f}, epoch={self.epoch}, decay={self.decay}, "
            f"ensemble_number={self.num_ens}, layer_name={self.feature_layer}, eta={self.eta:.4f}."
        )

        dummy_model_name = 'resnet18'  # Placeholder for TransferAttack's init

        self.attack_fn = TransferAttackP2FA(
